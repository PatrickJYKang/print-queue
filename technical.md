 ...
 **Text Description**
 ...
 - **Start window** Printer must report PRINTING within the default 30 minutes (configurable). If not, the reservation is released and the next user is notified.
...
flowchart TD
  ...
  W["Printer Watcher: any printer becomes IDLE"] --> N["Notify next user to claim"]
 ...
 **Text**
 ...
 - **Configuration** `.env` for Discord token and settings. `printers.yaml` for printer hosts/IDs. Timeouts (configurable): claim default 20 min, start default 30 min. Access hours: if a print finishes outside access hours, start countdowns at the next access period.
 - **Data Model (SQLite)**  
 ...
 - **Queue Manager Module** FIFO across all printers, persistence in SQLite, idempotent operations, recompute ETAs using user self-reported `est_minutes`, enforce configurable (more forgiving) claim/start timeouts, respect access hours by deferring countdowns to the next access period, crash-safe recovery on restart, concurrency-safe reservations.
